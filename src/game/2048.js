export default class Game2048 {
  ...
  initEvent() {
    ...
    function start(event) {
      x1 = event.clientX || event.changedTouches[0].clientX;
      y1 = event.clientY || event.changedTouches[0].clientY;
      if (event.which == 1) {
        this.mouseleft = true
      }
    }
    function end(event) {
      ...
      disX = (event.clientX || event.changedTouches[0].clientX) - x1;
      disY = (event.clientY || event.changedTouches[0].clientY) - y1;
      if (Math.abs(disX) > 10 || Math.abs(disY) > 10) {
        if (Math.abs(disX) > Math.abs(disY)) {
          _this.move(_this.arr, disX > 0 ? true : false);
        } else {
          _this.move(_this.getV2HArr(_this.arr), disY > 0 ? true : false, true);
        }
      }
    }
  }
